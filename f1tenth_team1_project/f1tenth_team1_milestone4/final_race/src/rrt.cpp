...
RRT::RRT(ros::NodeHandle &nh): nh_(nh), gen((std::random_device())()) {
    ...
    std::ifstream fin("<ADD LOCAL ADDRESS HERE>/src/final_race/src/skirk.csv");
    ...
    //read waypoint
    std::ifstream fin2("<ADD LOCAL ADDRESS HERE>/src/final_race/src/new_latest_traj.csv");
...
std::vector<double> RRT::sample(int goal_submap_x, int goal_submap_y) {
    ...
    std::mt19937 gen(rd());
	int sample_range = 40;
	if(pow(curr_.x - opp_.x, 2) + pow(curr_.y - opp_.y, 2) <= 9){sample_range = 60;}
