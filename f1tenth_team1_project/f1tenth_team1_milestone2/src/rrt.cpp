...
RRT::RRT(ros::NodeHandle &nh): nh_(nh), gen((std::random_device())()) {
    ...
    // opp_sub = nh_.subscribe(opp_odom_topic, 1, &RRT::opp_pos_callback, this);
    ...
    std::ifstream fin("/home/shivangimisra/shivangi_ws/src/f1tenth_team1_milestone2/src/skirk.csv");
    ...
    //read waypoint
    std::ifstream fin2("/home/shivangimisra/shivangi_ws/src/f1tenth_team1_milestone2/src/wp.csv");
...
void RRT::scan_callback(const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
    ...
    map_y = (abs(map_origin_y_) - curr_.y-2.4)/map_resolution_; 
    submap.clear();
    for(int i=0; i<80; i++){
        std::vector<float> row3;
		for(int j=0; j<80; j++){
	    	row3.push_back(map_global_[map_y-40+j][map_x-40+i]);
		}
...
//////////////////////////////////////////////////////////////////////
	...
	int row = 40+new_dist[j](1)/map_resolution_;
	int col = 40+new_dist[j](0)/map_resolution_;
	
	//update submap
	if((row <= 75 && row >= 4) && (col >= 4 && col <= 75)){
		for(int m=0;m<4;m++){
			for(int n=0;n<8;n++){
				if((79-(col-10+m) <= 79 && 79-(col-10+m) >= 0) && (79-(row-10+n) >= 0 && 79-(row-10+n) <= 79)){
				submap[79-(row-4+n)][79-(col-4+m)] = 0.0;
...
if(ctn == 0){
	...
	for(int m=0; m<80; m++){
			for(int n=0; n<80; n++){
				...
				p.x = curr_.x + (m - 40)*map_resolution_;
				p.y = curr_.y - (n - 40)*map_resolution_;
				p.z = 0;
    ...
    int goal_submap_x = 40 + goal_map_x - map_x;
    int goal_submap_y = 40 + goal_map_y - map_y;
...
/////////////////////////////////////////////////////////////
	...
	pose.pose.position.x = curr_.x + (goal_submap_x - 40)*map_resolution_;
	pose.pose.position.y = curr_.y - (goal_submap_y - 40)*map_resolution_;
	pose.pose.orientation.w = 1;
	temp_plan.push_back(pose);

	pose.pose.position.x = curr_.x + (new_node.x - 40)*map_resolution_;
	pose.pose.position.y = curr_.y - (new_node.y - 40)*map_resolution_;
	pose.pose.orientation.w = 1;
	temp_plan.push_back(pose);
	...
	pose.header.frame_id = "map";
	pose.pose.position.x = curr_.x + (tree[curr_parent].x - 40)*map_resolution_;
	pose.pose.position.y = curr_.y - (tree[curr_parent].y - 40)*map_resolution_;
	pose.pose.orientation.w = 1;
	temp_plan.push_back(pose);
...
////////////////////////////////////////
	...
	if(!tree[i].is_root){
		...
		geometry_msgs::Point p;
		p.x = curr_.x + (tree[i].x - 40)*map_resolution_;
		p.y = curr_.y - (tree[i].y - 40)*map_resolution_;
		p.z = 0;
		tree_path.points.push_back(p);
		p.x = curr_.x + (tree[tree[i].parent].x - 40)*map_resolution_;
		p.y = curr_.y - (tree[tree[i].parent].y - 40)*map_resolution_;
		tree_path.points.push_back(p);
	}
    }
...
/////////////////////////////////////////////////////////////////////////////////////////////
	...
	point.x = curr_.x + (tree[i].x - 40)*map_resolution_;
	point.y = curr_.y - (tree[i].y - 40)*map_resolution_;
...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    x_m << (found_path[found_path.size()-3].x-40)/20 + curr_x, -(found_path[found_path.size()-3].y-40)/20 + curr_y;//7?
    }else{
	x_m << (found_path[found_path.size()-2].x-40)/20 + curr_x, -(found_path[found_path.size()-2].y-40)/20 + curr_y;
    }
    x_c << curr_x, curr_y;
...
std::vector<double> RRT::sample(int goal_submap_x, int goal_submap_y) {
    ...
    std::mt19937 gen(rd());
    std::uniform_real_distribution<double> x_dist(0, 80);//30
    std::uniform_real_distribution<double> y_dist(0, 80);
    
    gen_x = x_dist(gen);
    gen_y = y_dist(gen);
    int cnt = 0;

    while(gen_x >= 80 || gen_y >= 80 || gen_x < 0 || gen_y < 0){
	gen_x = x_dist(gen);
    	gen_y = y_dist(gen);
    }
